...
function PatientDetail() {
  ...
  useEffect(() => {
    ...
    const fetchResults = async () => {
      // Corrected: Added 'hsv_stats' to the select query to retrieve the JSONB data
      const { data, error } = await supabase
        .from('results')
  ...
  // Helper function to render the color stats with simple styling
    ...
    return (
      <div style={{ display: 'flex', gap: '1rem', marginTop: '0.5rem', flexWrap: 'wrap' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          <div style={{ width: '16px', height: '16px', backgroundColor: 'red', borderRadius: '4px' }}></div>
          <p style={{ margin: 0, fontSize: '0.9rem' }}>
            <strong style={{ color: 'red' }}>Red:</strong> {stats.red_area_percent}%
          </p>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          <div style={{ width: '16px', height: '16px', backgroundColor: 'yellow', borderRadius: '4px' }}></div>
          <p style={{ margin: 0, fontSize: '0.9rem' }}>
            <strong style={{ color: 'goldenrod' }}>Yellow:</strong> {stats.yellow_area_percent}%
          </p>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          <div style={{ width: '16px', height: '16px', backgroundColor: 'black', borderRadius: '4px' }}></div>
          <p style={{ margin: 0, fontSize: '0.9rem' }}>
            <strong style={{ color: 'black' }}>Black:</strong> {stats.black_area_percent}%
          </p>
        </div>
      </div>
    );
